    ...
    def __init__(self, phase, topic, model='bert-base', wiki_model=''):
        ...
        if wiki_model:
            ...
            if wiki_model == model:
                ...
                tokenizer_senti = AutoTokenizer.from_pretrained('cardiffnlp/twitter-roberta-base-sentiment-latest')

            if wiki_model == model: #VAST
                ...
                #Senti below
                encodings_senti = tokenizer_senti(tweets, targets, padding=True, truncation=True)
        ...
        #senti below
        print(f'max len: {input_ids.shape[1]}, max len wiki: {input_ids_wiki.shape[1]}, max len senti: {input_ids_senti.shape[1]}',)
    ...
    def __init__(self, phase, model='bert-base', wiki_model=''):
        ...
        if wiki_model:
            ...
            if wiki_model == model:
                ...
                encodings_wiki = {'input_ids': [[0]] * df.shape[0], 'attention_mask': [[0]] * df.shape[0]}
                #Senti below
                encodings_senti = tokenizer_senti(tweets_targets, topics, padding=True, truncation=True)
            else:
